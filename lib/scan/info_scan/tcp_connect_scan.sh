...
function tcp_connect_scan()
{
	services "TCP Connect()"
  ...
  if [ $? -ne 0 ]; then
	...
	dir_input
	create_dir "$dir_name/tcp"


	$is_tor nmap -sT -p"$ports" -sV \
	 -f --send-eth -D RND:3 \
	 --script="$scripts" --script-args="$args" $debug_flag \
	 -oN "$HOME/$dir_name/tcp/$dns.nmap" "$dns"

	success_output "$HOME/$dir_name/tcp/$dns.nmap"
}
